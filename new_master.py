...
train_accuracy = pd.DataFrame({'Train_accuracy(%)': [train_accuracy(regressor), train_accuracy(LR),
                                                     train_accuracy(LR_PCA), train_accuracy(dtree), train_accuracy(rfc),
                                                     train_accuracy(dt_entr), train_accuracy(rf_entr),
                                                     train_accuracy(BernNB), train_accuracy(svc)]})
train_accuracy.index = ['LinearReg', 'LogReg', 'LogReg_PCA', 'Dec_tree', 'RandForr', 'Dec_tree_entr', 'RandFor_entr',
                        'BernNB', 'SVC']
...
x_val_score = pd.DataFrame({'X_val_score(%)': [x_val_score(LR), x_val_score(LR_PCA), x_val_score(dtree),
                                               x_val_score(rfc), x_val_score(dt_entr), x_val_score(rf_entr),
                                               x_val_score(BernNB), x_val_score(svc)]})
x_val_score.index = ['Lin_Reg', 'LogReg_PCA', 'Dec_tree', 'RandForr', 'Dec_tree_entr', 'RandFor_entr', 'BernNB', 'SVC']
...
#import matplotlib.pyplot as plt
#
mon_1 = ['Random Forrest', 'Supprt Vector Machine', 'Naive Bays', 'Logistic Regression','Logistic regression with PCA', 'Decision Tree', 'Perceptron', 'Linear Regression']
...
plt.barh(y_pos, mon_2, align='center', alpha=0.5)
...
f1 = ['Random Forrest', 'Supprt Vector Machine']
...
sortedcv_money = cv_money.sort_values(by='Train_accuracy(%)', ascending=False)
print('**Training Accuracy of the Classifiers:**')
